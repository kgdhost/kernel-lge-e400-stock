...
static void gpio_switch_work(struct work_struct *work)
{
	...
	int value;
//	int i;
	struct lge_gpio_switch_data	*data = lge_switch_data;
...
//	schedule_work(&switch_data->work);
	schedule_delayed_work(&switch_data->work, msecs_to_jiffies(150));
...
//	schedule_work(&switch_data->key_work);
	schedule_delayed_work(&switch_data->key_work, msecs_to_jiffies(30));
	return IRQ_HANDLED;
}
...
//	INIT_WORK(&switch_data->key_work, key_gpio_switch_work);
	INIT_DELAYED_WORK(&switch_data->key_work, key_gpio_switch_work);
...
static int __devexit lge_gpio_switch_remove(struct platform_device *pdev)
{
	...
	flush_delayed_work_sync(&switch_data->work);
	switch_dev_unregister(&switch_data->sdev);
